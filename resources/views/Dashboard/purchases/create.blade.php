        ...
        <div class="content-header">
            <div class="container-fluid">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-3">
                            <label for="contact">{{ trans('admin.contact') }} : </label>
                            ...
                            <select name="contact_id" id="contact_id" class="form-control select2 contact_id" required>
                                ...
                                <option value="{{$contact->id}}" @if (isset($sell) && $sell->contact_id == $contact->id) selected @endif>{{$contact->name}}  ({{$contact->balance}})</option>
                    ...
                    <div class="col-sm-3">
                        ...
                        <select name="delivery_status" id="delivery_status" class="form-control require" required>
                        ...
                        success: function(response) {
                            ...
                            if (existingRow.length > 0) {
                                ...
                                existingRow.find('.available-quantity').text(response
                                    .available_quantity);
                                existingRow.find('.total').text((newQuantity *response.purchase_price).toFixed(2));
                            } else {
                                // Add a new row
                                let newQuantity = 1;
                                let available_quantity = response.available_quantity;
                                let last_purchase_price = response.last_purchase_price ?
                                "اخر سعر شراء : " + response.last_purchase_price : "";
                                let warehouses = '';
                                ...
                                if (response.warehouses && response.warehouses.length > 0) {
                                    ...
                                    <td>
                                        <select id="warehouse" class="form-control" name="products[${response.id}][warehouse_id]">
                                            <option value="">الفرع (${response.availableQuantityBranch})</option>
                                            ${response.warehouses
                                                .map(
                                                    (warehouse) =>
                                ...
                                $('.sell_table').append(`
                                    <tr data-product-id="${response.id}">
                                                    ...
                                                    .map(
                                                        (unit) =>
                                                            `<option value="${unit.id}" data-multipler="${unit.multipler}" ${
                                                                    response.unit == unit.id ? 'selected' : ''
                                        ...
                                        <td class="available-quantity">${available_quantity}</td>
                                        <td>
                                            <input id="unit_price" type="number" class="form-control unit-price" 
                                                name="products[${response.id}][unit_price]" 
                                                value="${ response.purchase_price}" min="0" step="1">
                                                <p class="last-price-sale text-primary">${last_purchase_price}</p>
                                        </td>
                                        ...
                                        <input type="hidden" id="id" name="products[${response.id}][id]" value="${response.id}">
                                        <input type="hidden" id="main_unit_price" class="main_unit_price_${response.id}" name="products[${response.id}][main_unit_price]" value="${response.purchase_price}">
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.button-send', function(e) {
                ...
                tableRr.each(function(tr) {
                    ...
                    product.warehouse_id = warehouse.val();
