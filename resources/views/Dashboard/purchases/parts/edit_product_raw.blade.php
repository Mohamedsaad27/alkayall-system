<tr class="product_row_{{$product_row['id']}}">
    ...
    <td>    
        <select class="form-control select2 unit_change" name="products[{{$product_row['id']}}][unit_id]" required data-product_row_id="{{$product_row['id']}}">
            @if(is_array($product_row['units']) || is_object($product_row['units']))
                @foreach ($product_row['units'] as $unit)
              
                <option value="{{$unit['id']}}" data-multipler="{{$unit->getMultiplier()}}" @if(isset($product_row['unit_id']) && $product_row['unit_id'] == $unit['id']) selected @endif>{{$unit['actual_name']}}</option>
                @endforeach
            @endif
        </select>   
    </td>
