    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            ...
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('dashboard.home')}}">{{ trans('admin.Home') }}</a> / {{ trans('admin.villages') }}</li>
            </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
        </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-header">
            @if (auth('user')->user()->has_permission('create-villages'))
              <a href="{{route('dashboard.villages.create')}}" type="button" class="btn btn-info">{{ trans('admin.Add') }}</a>
            @else
...
<script type="text/javascript">
    var table = $('.data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
          "url": "{{ route('dashboard.villages.index') }}",
          "data": function ( d ) {
            d.role = $('#role').val();
          }
        },
