...
@section('title', trans('admin.Over Time'))
...
@section('content')
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
            <h1 class="m-0">{{ trans('admin.Over Time') }}</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('dashboard.home')}}">{{ trans('admin.Home') }}</a> / {{ trans('admin.Over Time') }}</li>
            </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
        </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-header">
            @if (auth('user')->user()->has_permission('create-overtime'))
              <a href="{{route('dashboard.hr.overtime.create')}}" type="button" class="btn btn-info">{{ trans('admin.Add') }}</a>
            @else
...
<script type="text/javascript">
    var table = $('.data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
          "url": "{{ route('dashboard.hr.overtime.index') }}",
          "data": function ( d ) {
