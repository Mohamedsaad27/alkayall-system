...
@section('title', trans('admin.activityTypes'))
...
@section('content')
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
            <h1 class="m-0">{{ trans('admin.activityTypes') }}</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('dashboard.home')}}">{{ trans('admin.Home') }}</a> / {{ trans('admin.activityTypes') }}</li>
            </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="card">
          <div class="card-header">
            @if (auth('user')->user()->has_permission('create-activityTypes'))
              <a href="{{route('dashboard.activityTypes.create')}}" type="button" class="btn btn-info">{{ trans('admin.Add') }}</a>
            @else
...
<script type="text/javascript">
    var table = $('.data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
          "url": "{{ route('dashboard.activityTypes.index') }}",
          "data": function ( d ) {
            d.role = $('#role').val();
          }
        },
