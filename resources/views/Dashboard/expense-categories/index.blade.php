...
@section('title', trans('admin.expense_categories'))
...
@section('content')
    ...
    <div class="content-header">
        ...
        <div class="row mb-2">
            <div class="col-sm-6">
            <h1 class="m-0">{{ trans('admin.expense_categories') }}</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('dashboard.home')}}">{{ trans('admin.Home') }}</a> / {{ trans('admin.expense_categories') }}</li>
            </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    ...
    <!--start filter-->
    {{-- @include('Dashboard.expense-categories.filter') --}}
    <!--end filter-->
      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-header">
            @if (auth('user')->user()->has_permission('create-expense-categories'))
              <a href="{{route('dashboard.expense-categories.create')}}" type="button" class="btn btn-info">{{ trans('admin.Add') }}</a>
            @else
...
<script type="text/javascript">
    var table = $('.data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
          "url": "{{ route('dashboard.expense-categories.index') }}",
          "data": function ( d ) {
            d.role = $('#role').val();
          }
        },
